...
describe('<RiskList />', () => {
    it('displays list of all systems when all risk lenses chosen', () => {
        const { getAllByText } = render(<BrowserRouter><RiskList systems={test_data} riskLens='combined' /></BrowserRouter>)
    ...
    it('displays list of systems when sunset risk lenses chosen', () => {
        const { getAllByText } = render(<BrowserRouter><RiskList systems={test_data} riskLens='sunset' /></BrowserRouter>)
        const systemElements = getAllByText(/Sys Name/)
        expect(systemElements.length).toEqual(4)
    })
    ...
    describe('when showing risk for a specific risk lens', () => {
        ...
        it('shows unknown risk when sunset is chosen', () => {
            const { getByTestId } = renderRiskList(test_data, 'portfolio', 'Portfolio A', 'sunset')
            const element = getByTestId('risk_cell_Sys Name 2')
    ...
    describe('when showing risk for all risk lenses', () => {
        it('shows shows System 3 as mediumHigh risk', () => {
            const { getByTestId } = renderRiskList(test_data, 'str', 'str', 'combined')
            const element = getByTestId('risk_cell_Sys Name 3')
...
function renderRiskList(systems = test_data, rootType = 'portfolio', rootValue = 'Portfolio A', riskLens = 'roadmap') {
    return render(
        <BrowserRouter>
            <RiskList systems={systems} rootType={rootType} rootValue={rootValue} riskLens={riskLens} />
        </BrowserRouter>)
}
