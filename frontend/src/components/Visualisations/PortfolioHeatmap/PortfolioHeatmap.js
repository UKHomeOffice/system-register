...
const PortfolioHeatmap = () => {
  ...
  const vis = systems?.length > 0 ? (<>
    ...
    <div className="visContainer">
      <div className="leftVis">
        <SystemsHeatmapVis systems={systems} view={selectedView} root={PORTFOLIO_ROOT} panelHeight="40"
        ...
        <FormControl component="fieldset">
          <RadioGroup name="view" value={selectedView} onChange={handleViewChange} row>
            <Tooltip title={knownRiskInfo}>
              <FormControlLabel data-testid="radioKnownRisk" value="knownRisk" control={<Radio />}
                label="Known risk only" />
            </Tooltip>
            <Tooltip title={includeUnknownRiskInfo}>
              <FormControlLabel data-testid="radioUnknownRisk" value="includeUnknownRisk" control={<Radio />}
                label="Include unknown risk" />
            </Tooltip>
            <Tooltip title={unknownsInfo}>
              <FormControlLabel data-testid="radioCoverage" value="unknowns" control={<Radio />}
  ...
  </>) : <div className="centerContent"><p>Loading visualisation...</p></div>;
