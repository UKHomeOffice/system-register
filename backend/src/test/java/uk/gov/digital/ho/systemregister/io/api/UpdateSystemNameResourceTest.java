...
public class UpdateSystemNameResourceTest extends ResourceTestBase {

    public UpdateSystemNameResourceTest(@SuppressWarnings("CdiInjectionPointsInspection") AgroalDataSource dataSource) {
        super(dataSource);
    }
    ...
    public void updatesSystemName() throws JSONException {
        ...
        String response = sendCommandToApi("update-name/command.json", "/api/systems/1/update-name", 200);
    ...
    public void mustBeAuthorised() {
        given().auth().none()
                .contentType(JSON)
                .body("{}")
                .when().post("/api/systems/1/update-name")
