...
public class UpdateSystemRiskResourceTest extends ResourceTestBase {
    ...
    @Test
    @TestSecurity
    public void updatesSystemRisk() throws JSONException {
        String expectedResponse = getResourceAsString("update-system-risk/updateSystemRiskSystemResponse.json");
        sendCommandToApi("update-system-risk/addSystemCommand.json", "/api/systems", 200);

        String response = sendCommandToApi("update-system-risk/command.json", "/api/systems/1/update-risk", 200);
    ...
    public void mustBeAuthorised() {
        given().auth().none()
                .contentType(JSON)
                .body("{}")
                .when().post("/api/systems/1/update-risk")
